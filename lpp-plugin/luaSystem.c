...
static int lua_openfile(lua_State *L)
{
	...
	SceUID id = sceIoOpen(device,mode,0777);
	lua_pushnumber(L,id);		
	return 1;
}
...
static int lua_seek(lua_State *L)
{
	...
	int position = luaL_checknumber(L, 3);
	int push = sceIoLseek(id,offset,position);	
...
static int lua_write(lua_State *L)
{
	...
	int id = luaL_checknumber(L, 1);
	char string[2048];
	strcpy(string,luaL_checkstring(L, 2));
	int size = luaL_checknumber(L, 3);
	sceIoWrite(id,string,size);
	return 0;
}
...
static int lua_runPSX(lua_State *L)
{
    if(lua_gettop(L) != 1)
	{
...
static int lua_getFileSize(lua_State *L)
{
	...
	SceUID id = sceIoOpen((char*)luaL_checkstring(L, 1),PSP_O_RDONLY,0777);
	int size = sceIoLseek(id,0,SEEK_END);
	sceIoClose(id);
	lua_pushnumber(1,size);
	return 1;
}

//Register our System Functions
static const luaL_reg System_functions[] = {
  {"getFileSize",                      lua_runPSX},
  {"startPSX",                      lua_runPSX},
  {"writeFile",                     lua_write},
  {"seekFile",                      lua_seek},
...
#define PSP_UTILITY_CONSTANT(name)\
	...
	int PSP_READ = 0x0001;
	int PSP_WRITE = 0x0002;
	int PSP_APPEND = 0x0100;
	int PSP_CREATE = 0x0200;
	int INIT_FILE = 0;
	int END_FILE = 2;
	int CUR_FILE = 1;
	VariableRegister(L, PSP_READ);
	VariableRegister(L, PSP_WRITE);
	VariableRegister(L, PSP_APPEND);
	VariableRegister(L, PSP_CREATE);
	VariableRegister(L, INIT_FILE);
	VariableRegister(L, END_FILE);
	VariableRegister(L, CUR_FILE);
}
