...
#include "../Libs/danzeff/danzeff.h"
...
extern "C" {
	...
	int getMbr(const char*);
	int setMbr(const char*);
	int setIpl(const char*);
	int getIpl(const char*);
	int getHen(void);
	int isSe(void);
	int isDevhook(void);
	#ifdef DEBUG
	...
	#endif
	extern int freemem();
	extern SceCtrlData *toControls(lua_State*,int);
}
...
static int luaSystem_listSaveInit(lua_State *L)
{
    ...
    if(args != 5 && args != 6)
    {
        return luaL_error(L, "System.listSaveInit(data, name, title, subtitle, detail, [cPath]) takes 5 or 6 arguments.");
    }
...
static int luaSystem_displayBrightness(lua_State *L)
{
    Int16 args = lua_gettop(L);
	if(args > 1) return luaL_error(L, "System.displayBrightness([value]) takes 1 or no arguments.");

	if(args)
	{
	    setDisplayBrightness(CLAMP(luaL_checknumber(L,1), 0, 100));
	}

	lua_pushnumber(L, getDisplayBrightness());
	return 1;
}

static int luaSystem_getRegParam(lua_State *L)
{
    if(lua_gettop(L) != 2)
	    return luaL_error(L, "System.getRegParam(dir, name) takes 2 arguments.");
    u32 val = 0;
	LPP_UtilsGetSystemReg(luaL_checkstring(L,1), luaL_checkstring(L,2), &val);
	lua_pushnumber(L, val);

	return 1;
}

static int luaSystem_setRegParam(lua_State *L)
{
    if(lua_gettop(L) != 3)
	    return luaL_error(L, "System.setRegParam(dir, name, value) takes 3 arguments.");
	int ret = LPP_UtilsSetSystemReg(luaL_checkstring(L,1), luaL_checkstring(L,2), luaL_checknumber(L,3));
	lua_pushboolean(L, !(ret < 0));

	return 1;
}

static int luaSystem_getIpl(lua_State *L)
{
    if(lua_gettop(L) != 1)
    {
        return luaL_error(L, "System.getIpl(out) takes 1 argument.");
    }
    int ret = getIpl(luaL_checkstring(L, 1));
    lua_pushboolean(L, !(ret < 0));

    return 1;
}

static int luaSystem_setIpl(lua_State *L)
{
    if(lua_gettop(L) != 1)
    {
        return luaL_error(L, "System.setIpl(in) takes 1 argument.");
    }
    int ret = setIpl(luaL_checkstring(L, 1));
    lua_pushboolean(L, !(ret < 0));

    return 1;
}

static int luaSystem_getMbr(lua_State *L)
{
    if(lua_gettop(L) != 1)
    {
        return luaL_error(L, "System.getMbr(out) takes 1 argument.");
    }
    int ret = getMbr(luaL_checkstring(L, 1));
    lua_pushboolean(L, !(ret < 0));

    return 1;
}

static int luaSystem_setMbr(lua_State *L)
{
    if(lua_gettop(L) != 1)
    {
        return luaL_error(L, "System.setMbr(in) takes 1 argument.");
    }
    int ret = setMbr(luaL_checkstring(L, 1));
    lua_pushboolean(L, !(ret < 0));

    return 1;
}

static int luaSystem_getHen(lua_State *L)
{
    lua_pushnumber(L, getHen());

    return 1;
}

static int luaSystem_isSe(lua_State *L)
{
    lua_pushboolean(L, isSe());
    return 1;
}

static int luaSystem_isDevhook(lua_State *L)
{
    lua_pushboolean(L, isDevhook());
    return 1;
}

static int luaSystem_danzeffInit(lua_State *L)
{
    danzeff_load();
    return 0;
}

static int luaSystem_danzeffFree(lua_State *L)
{
    danzeff_free();
    return 0;
}

static int luaSystem_danzeffReadInput(lua_State *L)
{
    if(lua_gettop(L) != 1)
    {
        return luaL_error(L, "System.danzeffReadInput(controls) takes 1 argument.");
    }
    char ret[] = {
        danzeff_readInput(*toControls(L,1)),
        '\0'
    };
...
static int luaSystem_danzeffMoveTo(lua_State *L)
{
    if(lua_gettop(L) != 2)
    {
        return luaL_error(L, "System.danzeffMove(x,y) takes 2 arguments.");
    }
    danzeff_moveTo(luaL_checknumber(L,1), luaL_checknumber(L,2));
    return 0;
}

static int luaSystem_danzeffDirty(lua_State *L)
{
    lua_pushboolean(L, danzeff_dirty());
    return 1;
}

static int luaSystem_danzeffRender(lua_State *L)
{
    danzeff_render();
    return 0;
}
...
static L_CONST luaL_reg luaSystem_methods[] = {
	...
	{ "displayBrightness", luaSystem_displayBrightness },
	{ "getRegParam", luaSystem_getRegParam },
	{ "setRegParam", luaSystem_setRegParam },
	{ "getIpl", luaSystem_getIpl },
	{ "setIpl", luaSystem_setIpl },
	{ "getMbr", luaSystem_getMbr },
	{ "setMbr", luaSystem_setMbr },
	{ "getHen", luaSystem_getHen },
	{ "isSe", luaSystem_isSe },
	{ "isDevhook", luaSystem_isDevhook },
	{ "danzeffInit", luaSystem_danzeffInit },
	{ "danzeffFree", luaSystem_danzeffFree },
	{ "danzeffDirty", luaSystem_danzeffDirty },
	{ "danzeffMove", luaSystem_danzeffMoveTo },
	{ "danzeffReadInput", luaSystem_danzeffReadInput },
	{ "danzeffRender", luaSystem_danzeffRender },
    { 0, 0 }
};
