...
void unSwizzleFast(Image *img)		
{
	...
	block = (void*)malloc(size);
	memcpy(block, img->data, size);
	unSwizzle_fast(temp, (u8*)img->data, (img->textureWidth * type), img->textureHeight );
	free(block);
...
Image* loadPngImageImpl(const char *filename)
{
	png_structp png_ptr;
	png_infop info_ptr;
...
Image* loadPngImage(const char* filename)
{
	...
	png_init_io(png_ptr, fp);
	Image* image = loadPngImageImpl(filename);
	fclose(fp);
	return image;
}
...
void initGraphics()
{
	...
	void *guBackBuffer = vrelptr(valloc(FRAMEBUFFER_SIZE));
	void *guFrontBuffer = vrelptr(valloc(FRAMEBUFFER_SIZE));
	void *guDepthBuffer = vrelptr(valloc(FRAMEBUFFER_SIZE/2));
	
	sceGuDrawBuffer(GU_PSM_8888, guFrontBuffer, PSP_LINE_SIZE);
	sceGuDispBuffer( 480, 272, guBackBuffer, PSP_LINE_SIZE);
	sceGuClear(GU_COLOR_BUFFER_BIT | GU_DEPTH_BUFFER_BIT);
	sceGuDepthBuffer(guDepthBuffer, PSP_LINE_SIZE);
