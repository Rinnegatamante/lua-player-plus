...
{
	if (lua_gettop(L) != 1) return luaL_error(L, "Speed of CPU Expected");
	int cpuspeed = luaL_checkint(L, 1);
	if (scePowerGetCpuClockFrequencyInt() != cpuspeed){
		if (cpuspeed > 333) {
			cpuspeed = 333;
		}
...
// Fixxed remove directories function
...
int fd = sceIoDopen(path);
	...
	while (sceIoDread(fd, &g_dir) > 0) {
		if (g_dir.d_name != "." & g_dir.d_name != ".." & g_dir.d_name != ""){
