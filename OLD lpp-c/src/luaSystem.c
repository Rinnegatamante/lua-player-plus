...
static int lua_SCEOsk(lua_State *L)
{
    if (sceKernelDevkitVersion() < 0x02000010)
   {
      luaL_error(L, "Only for FW 2.00 and Higher");
    return 0;
   } 
    if (lua_gettop(L) != 2) return luaL_error(L, "TXT in entry box needed,description of OSK needed Eg, 'Enter Name'");
    char *onlyName = luaL_checkstring(L, 1);
    char *description = luaL_checkstring(L, 2);
    char *newName = requestString(description, onlyName);
