...
#include "libs/sounds/aacplayer.h"
...
// aac  functions
bool aacplaying = false;

static int aac_load(lua_State *L)
{
	int argc = lua_gettop(L);
...
static int aac_play(lua_State *L)
{
	int argc = lua_gettop(L);
...
static int aac_Stop(lua_State *L)
{
	int argc = lua_gettop(L);
	if(argc != 0 ) return luaL_error(L, "wrong number of arguments");
	if (aacplaying == true){
...
static int aac_EndOfStream(lua_State *L)
{
	int argc = lua_gettop(L);
	if(argc != 0 ) return luaL_error(L, "wrong number of arguments");
	if (AAC_EndOfStream() == 1) { 
...
static int aac_getTime(lua_State *L)
{
	int argc = lua_gettop(L);
	if(argc != 0 ) return luaL_error(L, "wrong number of arguments");
    char aa3time[200];
	...
	if (aacplaying == true){
	AAC_GetTimeString(aa3time);
	sprintf(aa3timefeed, "%s", aa3time);
	lua_pushstring(L, aa3timefeed);
	}
	return 1;
}
static int aac_percent(lua_State *L)
{
	int argc = lua_gettop(L);
	if(argc != 0 ) return luaL_error(L, "wrong number of arguments");
	    char percent[200];
...
static int aac_pause(lua_State *L)
{
	int argc = lua_gettop(L);
	if(argc != 0 ) return luaL_error(L, "wrong number of arguments");
	if (aacplaying == true){
...
//Register our AAC Functions
static const luaL_reg Aac_functions[] = {
  {"load",				aac_load},
  {"play",				aac_play},
  {"stop",				aac_Stop},
  {"eos",				aac_EndOfStream},
